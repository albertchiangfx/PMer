...
router.post('/', async (req, res, next) => {
  try {
    const db = req.app.locals.db;
    const { project_id, contract_id, invoice_number, amount, currency, issued_date, due_date, status, notes, items } = req.body;
    if (!invoice_number || !amount || !issued_date) return res.status(400).json({ error: 'invoice_number, amount, issued_date are required' });
    ...
    try {
      ...
      `, [project_id, contract_id, invoice_number, amount, currency || 'USD', issued_date, due_date, status || 'draft', notes]);
      ...
      if (items?.length) {
        for (const item of items) {
          ...
          `, [invoice.id, item.team_member_id, item.task_id, item.description, item.hours, item.rate, item.amount]);
        }
      }
...
router.post('/generate', async (req, res, next) => {
  try {
    ...
    res.json({
      preview: { invoice_number: inv_num, project_id, contract_id, amount: total, items, issued_date, due_date }
    });
  } catch (e) { next(e); }
});
...
router.delete('/:id', async (req, res, next) => {
  try {
    const { rowCount } = await req.app.locals.db.query('DELETE FROM invoices WHERE id=$1', [req.params.id]);
...
router.get('/:id/pdf', async (req, res, next) => {
  try {
    ...
    const [inv, items, proj, client] = await Promise.all([
      ...
      `, [req.params.id]),
      db.query('SELECT * FROM projects WHERE id=(SELECT project_id FROM invoices WHERE id=$1)', [req.params.id]),
      db.query(`
    ...
    res.setHeader('Content-Disposition', `attachment; filename="invoice-${inv.rows[0].invoice_number}.pdf"`);
